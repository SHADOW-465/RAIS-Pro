...
function getPayload(e: Event): any {
  const { eventId, schemaVersion, ingestionId, occurredOn, provenance, confidence, extractedBy, recordedAt, supersededBy, eventType, ...rest } = e;
  return rest;
}
...
function mapRowToFinding(r: any): FindingT {
  return {
    findingId: r.finding_id,
    schemaVersion: r.schema_version,
    ingestionId: r.ingestion_id,
...
function mapRowToRule(r: any): RulebookRuleT {
  return {
    rulebookRuleId: r.rulebook_rule_id,
    version: r.version,
