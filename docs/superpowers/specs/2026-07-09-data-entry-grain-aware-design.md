...
- **Week (`grain === "week"`):** new `WeekPicker` component (new file, `src/components/WeekPicker.tsx`) — a small popover calendar. Clicking a day selects and highlights its Mon–Sun week row; the popover's own Prev/Next arrows move between months when browsing. Emits the week's `{ from, to }` ISO bounds.
